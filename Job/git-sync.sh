...
check_initial_file_state()
{
	...
	if [ ! -z "$(git status --porcelain | grep -E '^[^ \?][^M\?] *')" ]; then
	    __log_msg "There is a new files."
	    echo "NonNewOrModified"
	fi
    else
	# also bail on new files
	if [ ! -z "$(git status --porcelain | grep -E '^[^ ][^M] *')" ]; then
	    __log_msg "There is modified files."
...
"diverged")
	...
	if [[ $? == 0 && -z "$(git_repo_state)" && "ahead" == "$(sync_state)" ]] ; then
		...
		do
			...
			echo "Moving the file $flattenedfile that has conflict to C:\Conflicts"
			git show :3:"$file" > "C:/Conflicts/$flattenedfile"
		...
		#end for
		#git rebase by taking server changes
		#git commit, git push
		#Show message
	    exit 1
	fi
